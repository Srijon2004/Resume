...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="flex h-screen w-full flex-col items-center justify-center bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
      <Loader2 className="animate-spin text-blue-600 mb-4" size={48} />
      <p className="text-slate-500 dark:text-slate-400 font-medium animate-pulse tracking-wide">Loading your workspace...</p>
    </div>
  );
  ...
  return (
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 font-sans transition-colors duration-300">
      ...
      {/* NAVIGATION BAR: Added dark:bg-slate-900/80 and dark:border-slate-800 */}
      <nav className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b dark:border-slate-800 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-blue-600 rounded-lg text-white shadow-lg shadow-blue-200">
              <Sparkles size={20} />
            </div>
            <h1 className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tighter uppercase">AI Resume Builder</h1>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button 
              onClick={handleLogout}
              className="flex items-center gap-2 text-slate-400 dark:text-slate-500 hover:text-red-500 font-bold text-sm transition-colors"
            >
              <LogOut size={18} /> Logout
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-12">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            <h2 className="text-4xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Career Workspace</h2>
            <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Manage, optimize, and create professional resumes.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button 
              onClick={() => navigate("/checker")} 
              className="flex items-center gap-2 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 px-6 py-3 rounded-2xl font-bold border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm"
            >
              <Sparkles size={18} className="text-blue-500" /> AI Optimizer
            </button>
            <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
          {resumes.map((resume) => (
            <div 
              key={resume._id} 
              className="group bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden"
            >
              {/* HOVER ACTIONS */}
              <div className="absolute top-0 right-0 p-6 flex gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                <button 
                  onClick={() => navigate(`/editor?id=${resume._id}`)} 
                  className="p-3 bg-blue-50 dark:bg-slate-800 text-blue-600 dark:text-blue-400 rounded-xl hover:bg-blue-600 hover:text-white transition-colors"
                >
                  <Edit size={18} />
                </button>
                <button 
                  onClick={() => deleteResume(resume._id)} 
                  className="p-3 bg-red-50 dark:bg-slate-800 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-600 hover:text-white transition-colors"
                >
                  <Trash2 size={18} />
                </button>
              </div>
              ...
              <div className="mb-6">
                <div className="w-16 h-16 bg-blue-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-blue-600 dark:text-blue-400 mb-6 group-hover:scale-110 transition-transform duration-300">
                  <FileText size={32} />
                </div>
                <h3 className="font-extrabold text-slate-900 dark:text-slate-100 text-xl mb-2 truncate pr-16">
                  {resume.jobRole || "Untitled Resume"}
                </h3>
                <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm font-medium uppercase tracking-wider">
                  <Clock size={14} />
                  Created {new Date(resume.createdAt).toLocaleDateString()}
                </div>
              </div>
              ...
              <button 
                onClick={() => navigate(`/preview?id=${resume._id}`)}
                className="w-full mt-4 flex items-center justify-between px-6 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold text-slate-700 dark:text-slate-300 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300"
              >
                View Full Resume <ChevronRight size={18} />
              </button>
            </div>
          ...
          {/* EMPTY STATE */}
          {resumes.length === 0 && (
            <div className="col-span-full py-32 flex flex-col items-center justify-center bg-white dark:bg-slate-900 rounded-[3rem] border-4 border-dashed border-slate-100 dark:border-slate-800 shadow-inner">
              ...
              <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-200">Your workspace is empty</h2>
              <p className="text-slate-400 dark:text-slate-500 mb-8 max-w-sm text-center">Start building your professional career today by creating your first resume with AI insights.</p>
              <button 
                onClick={() => navigate("/create")} 
                className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 active:scale-95"
              >
...
function StatCard({ icon, label, value }) {
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 flex items-center gap-5 shadow-sm transition-shadow hover:shadow-md">
